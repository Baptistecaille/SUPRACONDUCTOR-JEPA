...
class JEPA(JEPAbase):
    """Trainable JEPA with prediction loss and without actions."""
    ...
    def __init__(self, encoder, predictor, regularizer, predcost):
        ...
        super().__init__(encoder, None, predictor)
    ...
    def forward(self, observations):
        """Forward pass used by ``nn.Module.__call__`` during training."""
        return self.predict(observations)

    def _split_context_target(self, state):
        """Split latent states into z_t context and z_{t+1} targets."""
        if state.dim() >= 5:
            return state[:, :, :-1], state[:, :, 1:]
        if state.dim() == 3:
            return state[:, :-1], state[:, 1:]
        raise ValueError(
            "JEPA expects encoded states with a temporal dimension, either "
            "[B, C, T, ...] or [B, T, D]. Got shape "
            f"{tuple(state.shape)}."
        )

    def predict(self, observations):
        """Compute JEPA training losses for a state-only sequence."""
        # Encode observations
        state = self.encoder(observations)

        # Compute regularization loss
        rloss, rloss_unweight, rloss_dict = self.regularizer(state, None)

        # Predict z_{t+1} from z_t
        context_states, target_states = self._split_context_target(state)
        predicted_states = self.predictor(context_states)

        # Compute prediction loss
        ploss = self.predcost(target_states, predicted_states)
...
class JEPA_AC(JEPAbase):
    ...
    def predict(
        ...
        """Predict exactly one latent step from an observation context.

        Args:
            observations: Observation context with shape [B, C, T, H, W].
            actions: Optional action sequence with shape [B, A, T_actions].
                The first action is used for the next transition, matching the
                one-step autoregressive path in ``unroll``.
            ctxt_window_time: Number of encoded context states to use for
                non-RNN predictors. RNN predictors always use one state.
            return_context: If True, return context plus prediction. Otherwise
                return only the predicted next latent state.

        Returns:
            Predicted latent state [B, D, 1, H', W'] by default, or
            [B, D, T_context + 1, H', W'] when return_context=True.
        """
        predicted_states, _ = self.unroll(
            observations,
            actions,
            nsteps=1,
